...
export default function PaymentForm({ onFormChange, formData = {} }) {
  
  const defaultFormData = {
    paymentType: formData.paymentType || "creditCard",
    cardNumber: formData.cardNumber || "",
    cvv: formData.cvv || "",
    cardName: formData.cardName || "",
    expDate: formData.expDate || "",
    saveCard: formData.saveCard || false,
    transactionId: formData.transactionId || "", // Nhận từ cha
  };

  const [localFormData, setLocalFormData] = React.useState(defaultFormData);
  const [errors, setErrors] = React.useState({});

  const paymentType = localFormData.paymentType;
  ...
  const handleExpirationDateChange = (event) => {
    ...
    if (value.length <= 4) {
      handleInputChange("expDate", formattedValue);
    }
  };
  ...
  const handleSaveCardChange = (event) => {
    handleInputChange("saveCard", event.target.checked);
  };
        ...
        <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
          ...
          <FormControlLabel
            control={
              <Checkbox
                name="saveCard"
                checked={localFormData.saveCard || false}
      ...
      {paymentType === "bankTransfer" && (
        <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
          ...
          <Box sx={{ display: "flex", gap: 1 }}>
            ...
            <Typography variant="body1" sx={{ fontWeight: "medium" }}>
              {import.meta.env.VITE_ROUTING_NUMBER}
            </Typography>
          </Box>
